...
class EnhancedHierarchicalClassifier:
    ...
    def _update_pattern_weights_restricted(self, description: str, remark: str, correct_path: List[str], boost: float):
        """Update pattern weights with restricted matching criteria"""
        # Store the exact description and remark for matching
        pattern_key = (description.lower().strip(), remark.lower().strip() if remark else "")
        path_key = tuple(correct_path)
        
        # Store in pattern weights with new structure
        if pattern_key not in self.pattern_weights:
            self.pattern_weights[pattern_key] = {}
        
        self.pattern_weights[pattern_key][path_key] = self.pattern_weights[pattern_key].get(path_key, 0.0) + boost
        
        self.logger.info(f"Updated restricted pattern: desc='{description[:30]}...', remark='{remark[:30] if remark else 'None'}...'")
        self.logger.info(f"Target path: {path_key}")
    ...
    def predict_with_confidence_enhanced(self, description: str, remark: str = "") -> Dict:
        ...
        # CHANGE: Use combined_text for prediction instead of calling predict_with_confidence
        result = self._predict_with_combined_text(combined_text)
        
        # CHANGE: Apply feedback boost BEFORE getting alternatives
        result = self._apply_feedback_boost_to_single_result(combined_text, result)
        
        # CHANGE: Get alternative paths using multi-path prediction
        try:
            multi_result = self.predict_multiple_paths(combined_text, "", top_k=3)
            alternative_paths = multi_result['top_paths'][1:] if len(multi_result['top_paths']) > 1 else []
        ...
        result.update({
            ...
            'reasoning': multi_result.get('reasoning', {}) if 'multi_result' in locals() else {},
            'debug_pattern_info': self.debug_pattern_matching(combined_text),
            'applied_feedback_boost': result.get('feedback_boost', 0.0)  # ADD this line
        })
    ...
    def _get_feedback_influenced_paths(self, text: str, top_k: int) -> List[Dict]:
        ...
        for (stored_desc, stored_remark), path_weights in self.pattern_weights.items():
            ...
            if stored_desc == input_desc.lower().strip():
                ...
                if remark_match:
                    for path_tuple, weight in path_weights.items():
                        if path_tuple not in path_scores:
                            path_scores[path_tuple] = 0.0
                        path_scores[path_tuple] += weight
...
def main_enhanced_combined():
    """Enhanced main function with combined text support - FIXED VERSION"""
    try:
        # Load training data
        print("Loading training data...")
        df = pd.read_excel('data/new_maximo_training_data_from_work_orders_cleaned.xlsx')
        print(f"Loaded {len(df)} training records")
        
        # FIXED: Prepare combined text FIRST before any other operations
        print("Preparing combined text from description and remark...")
        df = prepare_combined_text(df, desc_col='description', remark_col='remark')
        
        # Initialize enhanced classifier
        classifier = EnhancedHierarchicalClassifier(use_ensemble=True)
        
        # Load full hierarchy if available
        try:
            hierarchy_df = pd.read_excel('data/failure_paths_complete.xlsx')
            classifier.load_full_hierarchy(hierarchy_df)
            print("Loaded full Maximo hierarchy")
        except FileNotFoundError:
            print("Full hierarchy file not found - will use training data hierarchy only")
        ...
        print("Training enhanced hierarchical classifier with combined text...")
        classifier.fit(augmented_df)
        
        # Show hierarchy statistics
        stats = classifier.get_hierarchy_statistics()
        print("\n=== Hierarchy Statistics ===")
        print(f"Training hierarchy:")
        print(f"  - Failure classes: {stats['training_hierarchy']['failure_classes']}")
        print(f"  - Total paths: {stats['training_hierarchy']['total_paths']}")
        print(f"  - Average frequency: {stats['training_hierarchy']['avg_frequency']:.1f}")
        
        if 'full_hierarchy' in stats:
            print(f"Full hierarchy:")
            print(f"  - Failure classes: {stats['full_hierarchy']['failure_classes']}")
            print(f"  - Total paths: {stats['full_hierarchy']['total_paths']}")
        
        # Save the enhanced model
        classifier.save_model('enhanced_maximo_hierarchical_model.pkl')
        print("\nEnhanced model saved successfully!")
        
        # Example predictions with validation
        if len(df) > 0:
            print("\n=== Example Predictions ===")
            for i in range(min(3, len(df))):
                sample_text = df['description'].iloc[i]
                result = classifier.predict_with_validation(sample_text)
                
                print(f"\nText: '{sample_text}'")
                print(f"Predicted path: {' → '.join(result['complete_path'])}")
        ...
        print("\n✅ Enhanced hierarchical classifier training completed!")
